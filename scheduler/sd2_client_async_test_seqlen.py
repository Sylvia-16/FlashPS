...
async def run_inference_async(
    ...
    try:
        ...
        async with session.post(
            ...
            json={"inputs": inputs},
            timeout=500  # Set a longer timeout of 300 seconds (5 minutes)
        ) as response:
...
def run_inference_async_with_seqlen(
    ...
    inputs = {
        ...
        "image_path": edit_config.image_path,
        "mask_image_path": edit_config.mask_path,
        "strength": 1.0,
        "seed": 0,
...
if __name__ == "__main__":
    ...
    log_dir = log_dir + f"/sd2_client_flashps_{args.rps}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(log_dir, exist_ok=True)
