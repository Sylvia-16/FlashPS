...
from generate_seqlen import load_all_seqlen_files_ootd
# Setup logging
...
async def send_requests_async(
    ...
    timeout: int = 1800  # Default timeout of 30 minutes (1800 seconds)
):
...
def run_inference_async_with_seqlen(
    ...
    idx,
    request_timestamps,
    timeout: int = 1800  # Default timeout of 30 minutes
):
...
if __name__ == "__main__":
    ...
    log_dir = log_dir + f"/ootd_client_flashps_{args.rps}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(log_dir, exist_ok=True)
    ...
    edit_config_path = "/app/image-inpainting/configs/ootd_config_cb.yml"
    ...
    for i, seqlen_list in enumerate(seqlen_lists_stable):
        ...
        for j in range(test_time):
            ...
            results = run_inference_async_with_seqlen(seqlen_list, edit_config, args, i, request_timestamps, timeout=args.timeout)
