...
from generate_seqlen import load_all_seqlen_files,load_all_seqlen_files_experiment
# Setup logging
...
async def run_inference_async(
    ...
    try:
        ...
        async with session.post(
            ...
            json={"inputs": inputs},
            timeout=1000  # Set a longer timeout of 300 seconds (5 minutes)
        ) as response:
...
def run_inference_async_with_seqlen(
    ...
    inputs = {
        "prompt": "a sunflower under the sun",
        "num_inference_steps": 40,
        "mask_seq_length": 9216,  # Default value, will be overridden
    }
    inputs_list = []
    ...
    for i in range(len(request_timestamps)):
        ...
        print("mask_seq_length: ", new_inputs['mask_seq_length'])
    ...
    seqlen_list_logger.info(f"Time taken to send {args.rps * interval} requests with seqlen_list: {seqlen_list_total_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    # edit_config_path = "/home/xjiangbp/image-inpainting/configs/flux_inpaint_green_standard.yml"
    ...
    start_time = time.time()
    mask_ratio_trace_path = "/home/ubuntu/image-inpainting/scheduler/mask_ratio_trace_new.npy"
    mask_ratio_list = np.load(mask_ratio_trace_path)
    # 确保每次加载的seqlen_lists顺序一致
    seqlen_lists = []
    standard_seqlen= 9216
    seqlen_list = []
    ...
    for i, seqlen_list in enumerate(seqlen_lists_stable):
        ...
        for j in range(test_time):
            ...
            results = run_inference_async_with_seqlen(seqlen_list, args, i, request_timestamps)
