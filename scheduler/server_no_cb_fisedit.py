...
class DistributedWorker:
    def __init__(
        ...
        self.pipeline = StableDiffusionPipelineEdit.from_pretrained("/home/ubuntu/Hetu/sd2")
        self.pipeline.scheduler = DPMSolverMultistepScheduler.from_config(self.pipeline.scheduler.config)
        print("self.device", self.device)
        self.pipeline.to(self.device)
        # 从dist_config读取HetuUnetConfig参数
        hetu_config = self.dist_config.hetu_config if hasattr(self.dist_config, 'hetu_config') else {}
        
        self.config = HetuUnetConfig(
                height = hetu_config.get('height', 96),
                width = hetu_config.get('width', 96),
                ctx = self.pipeline.device,
                profile = hetu_config.get('profile', False),
                latent_scale_linear = hetu_config.get('latent_scale_linear', 24*24),
                latent_scale_conv = hetu_config.get('latent_scale_conv', 24*24),
                latent_scale_attn = hetu_config.get('latent_scale_attn', 48*48),
                fuse_gn_av_conv = hetu_config.get('fuse_gn_av_conv', 1),
                merge_rate = hetu_config.get('merge_rate', 0.9),
                fuse_attn1_attn2_ff = hetu_config.get('fuse_attn1_attn2_ff', 1),
                fuse_self_attn = hetu_config.get('fuse_self_attn', 1),
                fuse_cross_attn = hetu_config.get('fuse_cross_attn', 1),
                fuse_ln_ff_linear_av_add=hetu_config.get('fuse_ln_ff_linear_av_add', 1),
                radical_attn = hetu_config.get('radical_attn', True),
                radical_conv = hetu_config.get('radical_conv', True),
                strong_gpu = hetu_config.get('strong_gpu', False),
                turn_off_h2d = hetu_config.get('turn_off_h2d', False)
        )
        # run pipeline to get cache
        latents = torch.randn(1, 4, 96, 96).to(self.device)
        self.pipeline(prompt="test image", num_inference_steps=40, latents=latents, save_checkpoint=True, config=self.config)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("server mask_seq_length",mask_seq_length)
                ### Run the pipeline
                inference_start_time = time.time()
                self.logger.info(f"Running pipeline {pipeline_name} with for req_id: {message['req_id']}")
                latents = torch.randn(1, 4, 96, 96).to(self.device)
                images = pipeline(prompt=prompt, num_inference_steps=40, latents=latents, save_checkpoint=False, mask=None,mask_seqlen=mask_seq_length).images
...
if __name__ == "__main__":
    ...
    dist_config = DistributedConfig(
        nodes=node_configs, 
        port=config_data.get("port", 29500),
        hetu_config=config_data.get("hetu_config", {})
    )
